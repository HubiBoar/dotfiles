-- ~/.config/nvim/lua/plugins/simple_agent.lua
...
local function apply_relative(path)
  return relative_to(path, M.state.apply_root)
end
...
local function plan_prompt(user_text)
  return table.concat({
    ...
    "- Do not output replacement file blocks.",
    "- Do not output diffs.",
...
local function validate_replacement_file(file)
  ...
  local absolute = normalize(M.state.apply_root .. "/" .. path)
...
function M.open()
  ...
  M.state.apply_root = git_root_for(ctx.root)
